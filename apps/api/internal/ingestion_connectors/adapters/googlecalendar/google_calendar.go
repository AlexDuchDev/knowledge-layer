...
func (Adapter) ConnectorType() string { return "google_calendar" }
...
func (Adapter) ValidateSourceFeedConfig(ctx context.Context, connector ingestion_connectors.Connector, feed *ingestion_connectors.SourceFeed) error {
	...
	_ = connector
	if feed == nil {
		return fmt.Errorf("google_calendar: nil feed")
	}
	if feed.Status != "active" {
		return nil
	}
	return ingestion_connectors.ValidateGoogleCalendarSourceFeedForActivation(feed)
}
...
func (Adapter) SyncFeed(ctx context.Context, feed ingestion_connectors.SourceFeed, connector ingestion_connectors.Connector) error {
	...
	_ = feed
	_ = connector
	return fmt.Errorf("google_calendar adapter: use Service.SyncGoogleCalendar or queued connector:source_sync")
}

func (Adapter) MapArtifactMetadata(ctx context.Context, artifactType string, rawPayload []byte) (map[string]any, error) {
	_ = ctx
	m := map[string]any{"connector": "google_calendar", "artifact_type": artifactType}
	if len(rawPayload) > 0 {
